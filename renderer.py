...
# ─────────────────────────────────────────────────────────────
#  BLENDER 5.1 COMPATIBILITY PATCH
#  Changes from original eeVR:
#  1. Color space: 'Linear' / 'Linear Rec.709' → safe helper func
#  2. render.engine: 'BLENDER_EEVEE_NEXT' → 'BLENDER_EEVEE'
#  3. fb.read_color() slot param: 0 → 'COLOR' (Blender 4.0+ change)
#  4. bpy.app.version < (4,0,0) guards removed (we're on 5.x)
#  5. colorspace_settings assignment wrapped in try/except
# ─────────────────────────────────────────────────────────────

def _get_linear_colorspace():
    """Return the correct linear colorspace name for current Blender version."""
    candidates = ['Linear Rec.709', 'Linear', 'scene_linear']
    for name in candidates:
        try:
            # Test if this colorspace actually exists
            img_test = bpy.data.images.new('_cs_test', 4, 4)
            img_test.colorspace_settings.name = name
            bpy.data.images.remove(img_test)
            return name
        except Exception:
            try:
                bpy.data.images.remove(img_test)
                ...
                pass
    return 'Linear Rec.709'  # last resort fallback

LINEAR_COLORSPACE = None  # lazily initialized on first use

def get_linear_colorspace():
    global LINEAR_COLORSPACE
    if LINEAR_COLORSPACE is None:
        LINEAR_COLORSPACE = _get_linear_colorspace()
    return LINEAR_COLORSPACE


def set_colorspace_safe(image, name):
    """Set colorspace, falling back gracefully if name not found."""
    try:
        image.colorspace_settings.name = name
    except Exception:
        try:
            image.colorspace_settings.name = get_linear_colorspace()
        except Exception:
            pass  # leave as-is if all else fails


# ─────────────────────────────────────────────────────────────
#  Fragment shader definitions (unchanged from original eeVR)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
#  RENDERER CLASS — patched for Blender 5.1
# ─────────────────────────────────────────────────────────────
...
class Renderer:
    ...
    def __init__(self, context: bpy.types.Context, is_animation=False, folder=''):
        ...
        self.image_size = (
            int(ceil(self.scene.render.resolution_x * scale)),
            int(ceil(self.scene.render.resolution_y * scale))
        )
        ...
        fovfrac = (
            0.5 if props.fovModeEnum == '180'
            else 1 if props.fovModeEnum == '360'
            else max(h_fov, v_fov) / (2 * pi)
        )
        ...
        smargin = (
            0.0 if self.no_side_images or not vmargin > 0.0
            else max(0.0, 0.5 * (tan(pi / 4 + stitch_margin) - tan(pi / 4)))
        )
        ...
        frag_shader = \
            ...
            + ('' if self.no_back_image else (fetch_back % (
                (blend_seam_back_h if hmargin > 0.0 else '') +
                (blend_seam_back_v if vmargin > 0.0 else '')
            ))) \
            + (fetch_front % (
                (blend_seam_front_h if hmargin > 0.0 or ext_front_view else '') +
                (blend_seam_front_v if vmargin > 0.0 or ext_front_view else '')
            )) \
            + '}'
        ...
        def make_resolution(hscale, vscale, hmargin, vmargin, scale):
            return (
                int(ceil((base_resolution[0] * hscale + 2 * hmargin * base_resolution[0]) * scale)),
    ...
    def cubemap_to_panorama(self, imageList, outputName):
        ...
        # FIX: use safe colorspace helper instead of hardcoded version check
        textures = []
        for image in imageList:
            set_colorspace_safe(image, get_linear_colorspace())
        ...
        with offscreen.bind():
            ...
            # FIX: read_color 'slot' param changed from int (0) to string ('COLOR') in Blender 4.0+
            # Blender 5.x only accepts the string form
            try:
                buffer = fb.read_color(0, 0, width, height, 4, 0, 'FLOAT')
            except TypeError:
                # Fallback for older signature (should not occur on 5.1 but kept for safety)
                buffer = fb.read_color(0, 0, width, height, 4, 'COLOR', 'FLOAT')
        ...
        if outputName not in bpy.data.images.keys():
            bpy.data.images.new(outputName, width, height,
                                float_buffer=self.is_float, alpha=self.has_alpha)
    ...
    def set_camera_direction(self, direction):
        ...
        print(
            f"{direction} float:{self.is_float} alpha:{self.has_alpha} : "
            f"{self.scene.render.resolution_x} x {self.scene.render.resolution_y} "
            f"{degrees(self.camera.data.angle):.2f}° "
            f"[{self.camera.data.shift_x:.3f}, {self.camera.data.shift_y:.3f}] "
            f"({self.scene.render.pixel_aspect_x:.2f} : {self.scene.render.pixel_aspect_y:.2f}) "
            f"fstop={self.camera.data.dof.aperture_fstop:.2f}"
        )
        ...
        if self.is_stereo:
            ...
            if self.seamless and direction in {'right', 'left'}:
                ...
                self.camera.location = [
                    tmp_loc[0] + (0.5 * self.IPD * cos(camera_angle)),
                ...
                # FIX: safe colorspace assignment
                set_colorspace_safe(renderedImage, get_linear_colorspace())

                imageLen = len(renderedImage.pixels)
                renderedImageL = bpy.data.images.new(
                    nameL, self.scene.render.resolution_x, self.scene.render.resolution_y,
                    float_buffer=self.is_float, alpha=self.has_alpha
                )
                renderedImageR = bpy.data.images.new(
                    nameR, self.scene.render.resolution_x, self.scene.render.resolution_y,
                    float_buffer=self.is_float, alpha=self.has_alpha
                )
    ...
    def render_and_save(self):
        ...
        if self.is_stereo:
            ...
            if image_name not in bpy.data.images.keys():
                imageResult = bpy.data.images.new(
                    image_name, leftImage.size[0], 2 * leftImage.size[1],
                    float_buffer=self.is_float, alpha=self.has_alpha
                )
        ...
        if self.is_animation:
            ...
            imageResult.scene_set(self.scene.frame_current + frame_step) if hasattr(imageResult, 'scene_set') else \
        ...
        print(
            f"Saved '{imageResult.filepath_raw}' float:{self.is_float} alpha:{self.has_alpha}\n"
            f" Time : {round(time.time() - start_time, 2)}s "
            f"(Saving: {round(time.time() - save_start_time, 2)}s)\n"
        )
